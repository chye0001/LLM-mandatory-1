---
description: Implements the tickets in lane B
mode: subagent
---
...
1. Read `docs/tickets.md`. Work only tickets marked `Lane: B`.
...
Do not touch lane A tickets. Write no tests, the tester owns `tests/`. Write no
documentation, the doc writer owns it.

Done when every lane B ticket has all criteria ticked, or one blocker is
