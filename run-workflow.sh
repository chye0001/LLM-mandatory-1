...
Then produce exactly these files:
...
2. $DOC_DIR/openapi.yaml -- a valid OpenAPI 3.0 document covering every
   endpoint the brief implies, with request and response schemas and the error
   responses.
3. $DOC_DIR/adr/0001-<slug>.md and further numbered files -- one Architecture
...
API CONTRACT
$(read_artifact "$DOC_DIR/openapi.yaml")
...
- Every endpoint in the API contract must be covered by some ticket.
...
- $APP_DIR/requirements.txt pinning what it needs
- enough structure that the service starts with uvicorn from inside $APP_DIR

CONSTRAINTS
- Write only under $APP_DIR/. Do not touch the repository README.md,
  docker-compose.yml, litellm-config.yaml, $DOC_DIR/ or $ART_DIR/.
- Implement what the tickets say. Where a ticket is ambiguous, follow the API
  contract; where both are silent, choose the smaller option and note the
  choice in a code comment.
- The endpoint paths, methods, status codes and field names must match the API
  contract exactly.
...
TASK
1. Write pytest tests under $APP_DIR/tests/ covering every endpoint, including
   the not-found and validation-failure cases.
2. Actually run them. Run "python -m pytest" from inside $APP_DIR and read the
   output.
...
CONSTRAINTS
- Do not claim a test passed without having run it. This script re-runs pytest
  itself after you finish and prints the real exit code next to your report, so
  an inaccurate report is visible immediately.
...
1. $APP_DIR/README.md -- what the service is, how to install it, how to run it,
   and how to run its tests.
2. $DOC_DIR/api-usage.md -- a worked curl example of every endpoint, with a
   real request body and the response it returns.
3. $DOC_DIR/runbook.md -- how to operate it: start, stop, where the data lives,
   what the common failures look like and what to do about each.
...
- Do not document endpoints or options that are not implemented.
PROMPT_EOF
    ;;
...
1. $APP_DIR/Dockerfile -- builds and runs the service. Pin the base image tag.
   Do not run as root.
2. $APP_DIR/docker-compose.yml -- brings the service up, with the SQLite file
   on a named volume so data survives a restart.
3. $APP_DIR/.dockerignore
4. $DOC_DIR/deployment-checklist.md -- the ordered steps to deploy it, what to
   verify after each, and how to roll back.
...
- Do not modify the repository root docker-compose.yml. That file runs the
  toolchain's own gateway and has nothing to do with this service.
...
verify_phase() {
  ...
  tester)
    ...
    if ! command -v python >/dev/null 2>&1; then
      say "  python is not on PATH -- cannot verify. Treat the report as unverified."
      say ""
      return 0
    fi
    if [ ! -d "$APP_DIR" ]; then
    ...
    set +e
    ( cd "$APP_DIR" && python -m pytest -q ) 2>&1 | tee "$LOG_DIR/verify-pytest.log"
    prc=${PIPESTATUS[0]}
    set -e
    say ""
    if [ "$prc" -eq 0 ]; then
      say "  pytest exit 0 -- the suite passes."
    else
      say "  pytest exit $prc -- the suite does NOT pass."
